...
from megatron.model.transformer_gptj import TransformerLayerPipe, LinearPipe, EmbeddingPipe, Linear, NormPipe # #lsp
...
def _pre_transformer_block(args):
    ...
    # fn = lambda _args: (_args[0].transpose(0, 1).contiguous(), *_args[1:]) # 把input的batch size放到第二维度
    fn = lambda _args: _args # 把input的batch size放到第二维度
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    :param gptj_args: NeoX arguments object (configuration)
    ...
    def __init__(
        ...
        self.hidden_size = self.gptj_args.hidden_size
        ...
        super().__init__(
            layers=self.specs,
            loss_fn=partial(cross_entropy, _fp16=self.gptj_args.fp16_lm_cross_entropy),
            topology=topology,    # gxh
    ...
    def insert_layers(
        ...
        super().__init__(
            ...
            partition_method=self.gptj_args.pipe_partition_method,
            checkpointable_layers=["GMLPBlock", "TransformerLayerPipe"],
        )
    ...
    def init_specs(self):
        ...
        if weight_tying:
            self.specs.append(
                TiedLayerSpec(
                    ...
                    self.gptj_args,
                    self.hidden_size,
                    self.gptj_args.padded_vocab_size,
                    self.gptj_args.max_position_embeddings,
                    self.gptj_args.hidden_dropout,
                    self.init_method,
                    self.num_tokentypes,
        ...
        if self.gptj_args.pos_emb == "rpe":
            hidden_size_per_attention_head = self.gptj_args.hidden_size // self.gptj_args.num_attention_heads
            rpe_scale = math.sqrt(hidden_size_per_attention_head)
            rpe_emb = ParallelRelativePositionBias(    # TODO,?, gxh 
                gptj_args=self.gptj_args,
                scale=rpe_scale,
                causal=True,
                num_buckets=self.gptj_args.rpe_num_buckets,
                max_distance=self.gptj_args.rpe_max_distance,
                heads=self.gptj_args.num_attention_heads,
            )

        # Transformer layers
        for i in range(self.gptj_args.num_layers):
            layer_type = self.gptj_args.attention_config[i]
            if layer_type in ["gmlp", "amlp"]:
                ...
                self.specs.append(
                    LayerSpec(
                        ...
                        config=self.gptj_args,
                        # attention_mask_func=gpt2_attention_mask_func,    # gxh
                        # init_method=self.init_method,
                        # output_layer_init_method=self.output_layer_init_method,
                        # layer_number=i,
                        # rpe=rpe_emb if self.gptj_args.pos_emb == "rpe" else None,
                        # rotary=self.gptj_args.pos_emb == "rotary",
        ...
        norm, eps = get_norm(self.gptj_args)
        self.specs.append(
            LayerSpec(NormPipe, norm, self.gptj_args.hidden_size, eps=eps)
        )
    ...
    def _set_parallel_output(self, value):
        ...
        # @lsp
        # if isinstance(final_layer, (LinearPipe, Linear)):
